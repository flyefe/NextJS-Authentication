  ...
  if (Array.isArray(containers)) {
    for (const container of containers) {
      if (container === "LCL" && seaConfig.ratePerCBM && volume) {
        total += (seaConfig.ratePerCBM * volume * exchangeRate) + (seaConfig.customClearanceRatePerCBM ?? 0) + (seaConfig.documentationCost ?? 0);
        calculationDetails = `Formula: (ratePerCBM * volume * exchangeRate) + customClearanceRatePerCBM + documentationCost\nValues: (${seaConfig.ratePerCBM} * ${volume} * ${exchangeRate}) + (${seaConfig.customClearanceRatePerCBM ?? 0}) + (${seaConfig.documentationCost ?? 0})`;
      } else if (container === "20ft" && seaConfig.ratePer20ft) {
        total += (seaConfig.ratePer20ft * exchangeRate) + (seaConfig.customClearanceRatePerCBM ?? 0) + (seaConfig.documentationCost ?? 0);
      } else if (container === "40ft" && seaConfig.ratePer40ft) {
        total += (seaConfig.ratePer40ft * exchangeRate) + (seaConfig.customClearanceRatePerCBM ?? 0) + (seaConfig.documentationCost ?? 0);
      } else if (container === "40ftHighCube" && seaConfig.ratePer40ftHighCube) {
        total += (seaConfig.ratePer40ftHighCube * exchangeRate) + (seaConfig.customClearanceRatePerCBM ?? 0) + (seaConfig.documentationCost ?? 0);
      } else if (container === "45ftHighCube" && seaConfig.ratePer45ftHighCube) {
        total += (seaConfig.ratePer45ftHighCube * exchangeRate) + (seaConfig.customClearanceRatePerCBM ?? 0) + (seaConfig.documentationCost ?? 0);
      }
    }
  }
